...
async function build() {
    ...
    for (const icon of iconPackageJson.lsicon.icons) {
        const code = await transform(icon.fileContent, {
            icon: true,
            typescript: true,
        ...
        exportIcons.push(`export { default as ${icon.fileName} } from "./${icon.fileName}.tsx";`);
    }
    writeFile(path.resolve(iconComponentsPath, "index.ts"), exportIcons.join("\n"));
